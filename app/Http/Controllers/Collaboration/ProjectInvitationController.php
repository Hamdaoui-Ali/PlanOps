...
class ProjectInvitationController
{
    ...
    public function accept(Request $request, string $token, \App\Domain\Collaboration\Actions\AcceptProjectInvitation $accept): RedirectResponse
    {
    ...
    {
        $invitation = $invite->handle($request->user(), $project, $request->validated('email'), \App\Domain\Collaboration\Enums\ProjectRole::from($request->validated('role')));
